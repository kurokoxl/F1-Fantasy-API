...
namespace F1_Fantasy_API.Services.Interfaces
{
    ...
    {
        Task<Result<IEnumerable<DriverSelectionDto>>> GetDriverSelectionsAsync();
        Task<Result<IEnumerable<DriverSelectionDto>>> GetNextRaceSelections(string userId);
        Task<Result<DriverSelectionDto>> GetDriverSelectionByIdAsync(int driverId,string userId,int raceId);

        Task<Result<DriverSelectionDto>> AddDriverSelectionAsync(CreateDriverSelectionDto createDto,string userId);
        Task<Result<DriverSelectionDto>> UpdateDriverSelectionAsync(int id, UpdateDriverSelectionDto updateDto, string userId);
